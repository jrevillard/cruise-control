        ...
        switch ((KafkaAnomalyType) anomalyType) {
          case GOAL_VIOLATION:
            ...
            for (Entry<Boolean, List<String>> entry : violations.entrySet()) {
              entry.getValue().forEach( goal -> { 
                AlertaMessage alertaMessage = new AlertaMessage(localHostname, anomalyType.toString() + " - " + goalViolations.anomalyId());
          ...
          case BROKER_FAILURE:
            ...
            for (Entry<Integer, Long> entry : failedBrokers.entrySet()) {
              AlertaMessage alertaMessage = new AlertaMessage(localHostname + " - Broker " + entry.getKey(), anomalyType.toString() + " - " + brokerFailures.anomalyId());
              alertaMessage.setSeverity(severityFromPriority(anomalyType.priority()));
              alertaMessage.setGroup("Performance");
          ...
          case METRIC_ANOMALY:
            ...
            for (Entry<BrokerEntity, Long> entry : brokersEntitiesValues.entrySet()) {
              AlertaMessage alertaMessage = new AlertaMessage(localHostname + " - Broker " + entry.getKey().brokerId(), anomalyType.toString() + " - " + metricAnomaly.anomalyId());
              alertaMessage.setSeverity(severityFromPriority(anomalyType.priority()));
              alertaMessage.setGroup("Performance");
              alertaMessage.setCreateTime(outFormatter.format(Instant.ofEpochMilli(entry.getValue())));
              alertaMessages.add(alertaMessage);
          ...
          case DISK_FAILURE:
            ...
            failedDisks.forEach((brokerId, failures) -> {
              failures.forEach((logdir, eventTime) -> {
                AlertaMessage alertaMessage = new AlertaMessage(localHostname + " - Broker " + brokerId, anomalyType.toString() + " - " + diskFailures.anomalyId());
                alertaMessage.setSeverity(severityFromPriority(anomalyType.priority()));
          ...
          case TOPIC_ANOMALY:
            ...
            AlertaMessage alertaMessage = new AlertaMessage(localHostname, anomalyType.toString() + " - " + topicAnomaly.anomalyId());
            alertaMessage.setSeverity(severityFromPriority(anomalyType.priority()));
            alertaMessage.setGroup("Performance");
          ...
          case MAINTENANCE_EVENT:
            ...
            alertaMessage = new AlertaMessage(localHostname, anomalyType.toString() + " - " + maintenanceEvent.anomalyId());
            alertaMessage.setSeverity(severityFromPriority(anomalyType.priority()));
            alertaMessage.setGroup("Performance");
